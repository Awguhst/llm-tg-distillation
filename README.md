...
- `PILOT = True` in `config.py` sends only a handful of requests (2 generation requests,
...
- Sonnet 5 rejects `temperature` / `top_p` / `top_k`, so no sampling parameters are set.
  ...
  and a variant sentence in the prompt (see `common/prompts.py` and `config.py`).
- The two scripts that label polymers use the same short prompt from `common/prompts.py`.
